...
// Helper function to extract subtitle from children
function extractSubtitle(children: ReactNode): string | ReactNode | null {
	if (isValidElement(children)) {
		// Check for subtitle on the component's type
		const componentType = children.type as any;
		if (componentType && componentType.subtitle) {
			return componentType.subtitle;
		}
	}
	return null;
}
...
export default function AccessLayoutView({
	...
	const subtitle = extractSubtitle(children);
			...
			<SidebarInset className="flex flex-col p-2 transition-all duration-300 ease-in-out">
				...
				<div className="flex flex-1 flex-col gap-3 sm:gap-4 p-2 sm:p-4 lg:px-8 lg:py-6 transition-all duration-300 ease-in-out">
					<div className="w-full p-2 sm:p-3 lg:p-0 py-2 pb-3">
						...
						{/* Conditional rendering based on subtitle type */}
						{isDashboard ? (
