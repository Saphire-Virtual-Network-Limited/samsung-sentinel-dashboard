...
import { Button, Chip, Snippet, SortDescriptor, ChipProps } from "@heroui/react";
...
export default function SingleMBEPage() {
  ...
  useEffect(() => {
    ...
    const mbeData = safeRaw.find((c: MBERecord) => 
      c.mbeId === params.id || 
    ...
    const availableIds = safeRaw.map((c: MBERecord) => ({
      id: c.mbeId,
      type: typeof c.mbeId,
  ...
  const totalLoanAmount = mbe.Customers?.reduce((total, customer) => {
  ...
  const customersData = mbe.Customers ? mbe.Customers.map((customer) => ({
  ...
  const exportFn = async (data: any[]) => {
    ...
    ws.columns = columns.map((c) => ({ header: c.name, key: c.uid, width: 20 }));
    
    data.forEach((r) => ws.addRow(r));
    const buf = await wb.xlsx.writeBuffer();
    saveAs(new Blob([buf]), `MBE_Customers_${mbe.firstname || "Unknown"}_${mbe.lastname || "MBE"}.xlsx`);
  };

  // Render cell function
  const renderCell = (row: any, key: string) => {
    if (key === "loanAmount") {
      return `₦${row[key]?.toLocaleString() || "N/A"}`;
    }
    if (key === "dobStatus") {
      return (
        <Chip
    ...
    if (key === "createdAt") {
      return row[key] !== "N/A" ? new Date(row[key]).toLocaleDateString() : "N/A";
    }
    return row[key];
  };
          ...
          <div className="bg-white rounded-xl shadow-sm border border-default-200 p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-default-900">{customerCount}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-default-200 p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-default-900">{storeCount}</p>
              </div>
            </div>
          </div>
              ...
              <div className="p-4">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="font-medium text-default-900">
                      {mbe.createdAt
                        ? new Date(mbe.createdAt).toLocaleString()
                        : "N/A"}
                    </div>
                  </div>
                </div>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="bg-default-100">
                          <th className="px-4 py-2 text-left text-sm font-medium text-default-700">Store ID</th>
                          <th className="px-4 py-2 text-left text-sm font-medium text-default-700">MBE ID</th>
                          <th className="px-4 py-2 text-left text-sm font-medium text-default-700">MBE Old ID</th>
                          <th className="px-4 py-2 text-left text-sm font-medium text-default-700">Store Old ID</th>
                        </tr>
                      </thead>
                      <tbody>
                        {mbe.stores.map((store, index) => (
                          <tr key={index} className="border-t border-default-200">
                            <td className="px-4 py-3 text-sm text-default-900">{store.storeId || "N/A"}</td>
                            <td className="px-4 py-3 text-sm text-default-500">{store.mbeId || "N/A"}</td>
                            <td className="px-4 py-3 text-sm text-default-500">{store.mbeOldId || "N/A"}</td>
                            <td className="px-4 py-3 text-sm text-default-500">{store.storeOldId || "N/A"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
