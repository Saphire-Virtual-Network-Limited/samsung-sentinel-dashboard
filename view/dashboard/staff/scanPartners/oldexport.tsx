	...
	try {
				...
				const rowData = {
					...
					monthlyPayment:
						(customerDetails?.monthlyPayment ||
							...
							: "N/A",
					duration: customerDetails?.duration || loanRecord?.duration || "N/A",
		...
		commissionAgents.forEach((agent: any) => {
			...
			const rowData = {
				...
				totalPartnerCommission: totalPartnerCommission.toLocaleString("en-GB"),
				commissionCount,
				avgCommission:
		...
		for (const customer of customerRecords) {
			...
			const rowData = {
				...
				monthlyRepayment: customer.LoanRecord?.[0]?.monthlyRepayment
					? `${customer.LoanRecord[0].monthlyRepayment.toLocaleString("en-GB")}`
					: "N/A",
				amountRemaining:
				...
				spBankName: scanPartnerInfo?.UserAccountDetails?.[0]?.bankName || "N/A",
				spBankCode: scanPartnerInfo?.UserAccountDetails?.[0]?.bankCode || "N/A",
				spVfdBankName:
		...
		if (salesData && salesData.partnerStats) {
			...
			if (salesData.summary) {
				const summaryRow = performanceWs.addRow({
					...
					stateCount: "-",
					avgCommissionPerAgent: salesData.summary.averageAgentsPerPartner || 0,
					period: salesData.period || salesPeriod,
				});
