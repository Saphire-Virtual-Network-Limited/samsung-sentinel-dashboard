...
export default function DownPaymentView() {
	...
	React.useEffect(() => {
		console.log("Down Payment - Channel state changed to:", channel);
	}, [channel]);
	// --- table state ---
	...
	const {
		...
		["down-payment-data", fromDate, toDate, channel],
		() => {
			console.log("Fetching data with params:", { fromDate, toDate, channel });
			return getDownPaymentData(channel, fromDate, toDate)
				.then((r) => {
						...
						<div className="flex gap-2 w-full sm:w-auto">
								...
								<Dropdown>
									...
									<DropdownMenu
										...
										onSelectionChange={(keys) => {
											console.log("Down Payment - Raw keys from dropdown:", keys);
											const selectedKey = Array.from(keys)[0] as string;
											console.log("Down Payment - Selected channel:", selectedKey);
									...
									>
										{channelOptions.map((option) => (
											<DropdownItem 
												...
												value={option.uid}
												onPress={() => {
													console.log("Down Payment - Item pressed:", option.uid);
													if (option.uid === "") {
														setChannel(undefined);
													} else {
							...
							<div className="flex gap-2">
								<Button
									...
									size="sm"
									onPress={() => {
										console.log("Down Payment - Manually refreshing data...");
										mutate();
									}}
									className="px-3"
									isLoading={isLoading}
								...
								{channel && (
									<Button
										...
										size="sm"
										onPress={() => {
											console.log("Down Payment - Clearing channel filter");
											setChannel(undefined);
										}}
										className="px-3"
									>
