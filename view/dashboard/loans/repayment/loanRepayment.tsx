...
import { capitalize, calculateAge, getLoanRepaymentData } from "@/lib";
...
export default function LoanRepaymentView() {
	...
	const customers = useMemo(() => {
		...
		return customersData.map((customer: any, index: number): TransformedDueLoanRecord => {
		...
		if (filterValue) {
			...
			list = list.filter((c) => {
				try {
					...
					return customerName.includes(f) || 
	...
	const exportFn = async (data: TransformedDueLoanRecord[]) => {
		...
		const ws = wb.addWorksheet("Loan_Repayments");	
		ws.columns = exportColumns.map((c) => ({ header: c.name, key: c.uid, width: 20 }));
		data.forEach((r) => ws.addRow(r));		
		const buf = await wb.xlsx.writeBuffer();
		saveAs(new Blob([buf]), "Loan_Repayments.xlsx");
	};
	...
	const renderCell = (row: TransformedDueLoanRecord, key: string) => {
		...
		if (key === "amount") {
			return <div className="text-small">₦{row.amount?.toLocaleString() || '0'}</div>;
		}
		if (key === "createdAt") {
			return <div className="text-small">{new Date(row.createdAt).toLocaleDateString()}</div>;
		}
						...
						<div className="px-8 py-6 border-b border-gray-100/60">
							<div className="flex items-center justify-between">
								<div className="flex items-center space-x-3">
									...
									<div>
										<h2 className="text-xl font-bold text-gray-900 tracking-tight">Loan Repayment Summary</h2>
								...
								<div className="flex items-center space-x-2 bg-gray-50/80 px-4 py-2 rounded-xl border border-gray-200/60">
									...
									<span className="text-sm font-semibold text-gray-700">{startDate} - {endDate}</span>
								</div>
							</div>
						</div>
							...
							<div className="grid grid-cols-4 gap-8">
								...
								<div className="group relative">
									<div className="bg-gradient-to-br from-violet-50 to-violet-100/80 rounded-2xl p-6 border border-violet-200/40 transition-all duration-300 group-hover:shadow-lg group-hover:scale-105 group-hover:border-violet-300/60">
										...
										<p className="text-sm font-semibold text-violet-700 mb-2 tracking-wide">Total Repayments</p>
								...
								<div className="group relative">
									<div className="bg-gradient-to-br from-blue-50 to-blue-100/80 rounded-2xl p-6 border border-blue-200/40 transition-all duration-300 group-hover:shadow-lg group-hover:scale-105 group-hover:border-blue-300/60">
										...
										<p className="text-sm font-semibold text-blue-700 mb-2 tracking-wide">Repayment Count</p>
										<div className="min-h-[3rem] flex items-center justify-start mb-2">
											<p className="text-lg sm:text-xl lg:text-2xl xl:text-3xl font-bold text-gray-900 tracking-tight leading-tight break-words">
								...
								<div className="group relative">
									<div className="bg-gradient-to-br from-amber-50 to-amber-100/80 rounded-2xl p-6 border border-amber-200/40 transition-all duration-300 group-hover:shadow-lg group-hover:scale-105 group-hover:border-amber-300/60">
										...
										<p className="text-sm font-semibold text-amber-700 mb-2 tracking-wide">Average Amount</p>
										<div className="min-h-[3rem] flex items-center justify-start mb-2">
											<p className="text-base sm:text-lg lg:text-xl xl:text-2xl font-bold text-gray-900 tracking-tight leading-tight break-words">
												₦{raw.summary?.averageRepaymentAmount?.toLocaleString() || '0'}
											</p>
										</div>
									...
									<div>
										...
										<div className="space-y-1">
											<p className="text-sm font-semibold text-blue-800">
												CARD: {customers.filter((c: TransformedDueLoanRecord) => c.channel === 'card').length}
											</p>
											<p className="text-sm font-semibold text-blue-800">
												Dedicated NUBAN: {customers.filter((c: TransformedDueLoanRecord) => c.channel === 'dedicated_nuban').length}
											</p>
										</div>
									</div>
								</div>
