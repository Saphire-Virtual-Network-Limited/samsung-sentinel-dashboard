	...
	const customers = useMemo(
		() =>
			raw.map((r: LoanRecord) => ({
				...
				// monthlyRepayment: r.monthlyRepayment ? `₦${r.monthlyRepayment.toLocalString("en-GB")}` : 'N/A',
				tenorInDays: r.duration ? `${r.duration * 30} days` : "N/A",
				status: r.status || "N/A",
				loanAmount: r.loanAmount
					? `₦${r.loanAmount.toLocalString("en-GB")}`
					: "N/A",
				downPayment: r.downPayment
					? `₦${r.downPayment.toLocalString("en-GB")}`
					: "N/A",
				devicePrice: r.devicePrice
					? `₦${r.devicePrice.toLocalString("en-GB")}`
					: "N/A",
