...
import { createSchema, createDevice, updateDevice, showToast, getAllDevices } from '@/lib'  
import { FormField, InputFileBoxField, SelectField } from '@/components/reususables/' 
...
const DeviceForm = () => {
    ...
    const handleDeviceImageChange = (file: File | null) => {
        setDeviceImage(file);
        validateField(file ? file.name : '', deviceImageSchema, setDeviceImageError);
    };
    ...
    const handleWindowsVersionChange = (value: string | string[]) => {
        ...
        validateField(windowsVersionValue, windowsVersionSchema, setWindowsVersionError);
    };
        ...
        if (isEditMode && deviceId) {
            const fetchStoreData = async () => {
                try {
                    ...
                    const device = devices.find((d: DeviceRecord) => d.newDeviceId === deviceId);
                    
                    if (device) {
                        setDeviceData(device);
                        // Pre-fill form fields
                        setDeviceBrand(device.deviceBrand || '');
                        ...
                        setDeviceModelNumber(device.deviceModelNumber || '');
                        setBackCamera(device.back_camera || '');
                        setBattery(device.battery || '');
                        setColor(device.color || '');
                        setDataStorage(device.data_storage || '');
                        setDisplay(device.display || '');
                        setFrontCamera(device.front_camera || '');
                        setMemory(device.memory || '');
                        setNetwork(device.network || '');
                        setOs(device.os || '');
                        setOtherFeatures(device.other_features || '');
                        setProcessorCpu(device.processor_cpu || '');
                        setSap(device.sap?.toString() || '');
                        setScreenSize(device.screen_size || '');
                        setSld(device.sld?.toString() || '');
                        setDeviceType(device.deviceType || '');
                        setCaseColors(device.case_colors || '');
                        setWindowsVersion(device.windows_version || '');
                        setIsActive(device.isActive || false);
                        setSentinelCover(device.sentinel_cover || '');
                    }
                } catch (error: any) {
    ...
    const handleSubmit = async() => {
        ...
        try {
            ...
            showToast({
                type: "error", 
                message: error.message || (isEditMode ? "Device update failed" : "Device creation failed"), 
                duration: 8000
            });
        } finally {
            setIsLoading(false);
        ...
        <div className='flex flex-col gap-6'> 
            ...
            <div className="mb-4">
                <h1 className="text-2xl font-bold">
                    <span className="text-blue-600">{isEditMode ? `Edit ${deviceData?.deviceBrand} Device` : 'Create New Device'}</span>
                </h1>
            </div>
            
            {/* Basic Device Information */}
            <div className="bg-white p-6 rounded-lg border border-gray-200">
                <h2 className="text-lg font-semibold mb-4 text-gray-800">Basic Device Information</h2>
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200">
                <h2 className="text-lg font-semibold mb-4 text-gray-800">Physical Specifications</h2>
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                    <FormField
                        label="Display"
            ...
            {/* Camera & Battery */}
            <div className="bg-white p-6 rounded-lg border border-gray-200">
                <h2 className="text-lg font-semibold mb-4 text-gray-800">Camera & Battery</h2>
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                    <FormField
                        label="Back Camera"
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200">
                <h2 className="text-lg font-semibold mb-4 text-gray-800">Additional Features & Media</h2>
