...
import {  capitalize, calculateAge, getAllUnEnrolledDevices } from "@/lib";
...
export default function UnEnrolledDevicesView() {
	// --- modal state ---
	...
	const { data: raw = [], isLoading, error } = useSWR(
		startDate && endDate ? ["unenrolled-devices", startDate, endDate] : "unenrolled-devices",
		() => getAllUnEnrolledDevices("UNENROLLED", startDate, endDate)
			.then((r) => {
			...
			.catch((error) => {
				console.error("Error fetching unenrolled devices:", error);
				setHasNoRecords(true);
				return [];
			}),
		{
	...
	const exportFn = async (data: DeviceRecord[]) => {
		...
		const ws = wb.addWorksheet("Unenrolled_Devices");	
		...
		saveAs(new Blob([buf]), "Unenrolled_Devices.xlsx");
	};
