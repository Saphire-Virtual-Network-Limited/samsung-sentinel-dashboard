...
import { getAllReferees, capitalize, calculateAge, showToast, verifyCustomerReferenceNumber } from "@/lib";
...
type UnapprovedRefereeRecord = {
	...
	const { data: raw = [], isLoading } = useSWR(
		startDate && endDate ? ["all-referees", startDate, endDate] : "all-referees",
	...
	const renderCell = (row: UnapprovedRefereeRecord, key: string) => {
		...
		if (key === "fullName") {
			return <div className="capitalize cursor-pointer" onClick={() => openModal("view", row)}>{(row as any)[key]}</div>;
		}
