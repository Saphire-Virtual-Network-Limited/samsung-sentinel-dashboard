...
export default function SingleRefereeView({
	id,
	role = "verify",
	...
	const handleApproveReferee = async () => {
		...
		try {
			...
			showToast({
				type: "success",
				message:
					"Referee verified successfully, Please check approved referees",
				duration: 4000,
			});
			router.push(
				`/access/${role}/referees/approved-referees/${customer.customerId}`
			...
			showToast({
				type: "error",
				message: error.message || "Failed to verify referee",
				duration: 5000,
			});
		} finally {
	...
	const handleRejectReferee = async () => {
		...
		try {
			...
			showToast({
				type: "success",
				message:
					"Referee rejected successfully, Please check rejected referees",
				duration: 4000,
			});
			router.push(
				`/access/${role}/referees/rejected-referees/${customer.customerId}`
			);
		} catch (error: any) {
							...
							<div className="p-6">
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
									...
									<div className="bg-default-50 rounded-lg p-4">
										...
										<div className="font-medium text-default-900">
											{customer.LoanRecord?.[0]?.channel || "N/A"}
										</div>
									</div>
								</div>
							</div>
					...
					<div className="bg-white rounded-xl max-w-md w-full mx-4 shadow-xl transform transition-all">
						...
						<div className="p-6">
							<div className="mb-6 text-default-600">
								Are you sure you want to approve this referee?
							</div>
