...
export default function ApprovedRefereesPage() {
	...
	const [selectedItem, setSelectedItem] =
		useState<ApprovedRefereeRecord | null>(null);
	const [isButtonLoading, setIsButtonLoading] = useState(false);
	const [refereeType, setRefereeType] = useState<
		"referee1" | "referee2" | null
	>(null);
	...
	const { data: raw = [], isLoading } = useSWR(
		startDate && endDate
			? ["approved-refreees", startDate, endDate]
			: "approved-refreees",
		() =>
			getApprovedReferees(startDate, endDate)
				...
				.catch((error) => {
					console.error("Error fetching approved referees:", error);
					setHasNoRecords(true);
					return [];
				}),
		{
	...
	const customers = useMemo(
		() =>
			raw.map((r: ApprovedRefereeRecord) => ({
				...r,
	...
	const sorted = React.useMemo(() => {
		return [...paged].sort((a, b) => {
			const aVal = a[sortDescriptor.column as keyof ApprovedRefereeRecord];
			const bVal = b[sortDescriptor.column as keyof ApprovedRefereeRecord];
	...
	const handleApproveReferee = async (
		selectedItem: ApprovedRefereeRecord | null,
		type: "referee1" | "referee2" | null
	) => {
		...
		try {
			...
			console.error(
				`Failed to verify referee ${type === "referee1" ? "1" : "2"}`,
				error
			);
			showToast({
				type: "error",
	...
	const handleRejectReferee = async (
		...
		try {
			...
			console.error(
				`Failed to reject referee ${type === "referee1" ? "1" : "2"}`,
				error
			);
			showToast({
				type: "error",
	...
	// Export all filtered
	const exportFn = async (data: ApprovedRefereeRecord[]) => {
		const wb = new ExcelJS.Workbook();
		const ws = wb.addWorksheet("Approved Referees");
		ws.columns = columns
		...
		saveAs(new Blob([buf]), "Approved_Referees_Records.xlsx");
	};

	// When action clicked:
	const openModal = (mode: "view", row: ApprovedRefereeRecord) => {
		const role = pathname.split("/")[2] || "verify" || "admin" || "dev";
		router.push(`/access/${role}/referees/approved-referees/${row.customerId}`);
	};

	// Render each cell, including actions dropdown:
	const renderCell = (row: ApprovedRefereeRecord, key: string) => {
		if (key === "actions") {
			return (
	...
	return (
		<>
										...
										<div className="bg-default-50 p-4 rounded-lg">
											...
											<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
												...
												<div>
													...
													<p className="font-medium">
														{selectedItem.dobMisMatch === false ? "No" : "Yes"}
													</p>
												</div>
												<div>
													<p className="text-sm text-default-500">Reg by:</p>
													<p className="font-medium">
														{selectedItem.mbeId || "N/A"}
													</p>
												</div>
											</div>
										</div>
										...
										<div className="bg-default-50 p-4 rounded-lg">
											...
											<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
												<div>
													...
													<p className="font-medium">
														{selectedItem.CustomerKYC?.[0]?.applicantAddress ||
															"N/A"}
													</p>
												</div>
												<div>
													...
													<p className="font-medium">
														{selectedItem.CustomerKYC?.[0]
															?.applicantBusinessAddress || "N/A"}
													</p>
												</div>
												<div>
													<p className="text-sm text-default-500">Occupation</p>
													<p className="font-medium">
														{selectedItem.CustomerKYC?.[0]?.occupation || "N/A"}
													</p>
												</div>
												<div>
													<p className="text-sm text-default-500">
													...
													<p className="font-medium">
														{selectedItem.CustomerKYC?.[0]?.businessName ||
															"N/A"}
													</p>
												</div>
												...
												<div className="flex flex-col gap-4 bg-gray-200 p-4 rounded-lg">
													<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
														<div>
															...
															<p className="font-medium">
																{selectedItem.CustomerKYC?.[0]?.phone2 || "N/A"}
															</p>
														</div>
														...
														<div>
															...
															<p className="font-medium">
																{selectedItem.CustomerKYC?.[0]?.phone2Status ||
																	"N/A"}
															</p>
														</div>
														<div>
													...
													{selectedItem.CustomerKYC?.[0]?.phone2 &&
														selectedItem.CustomerKYC?.[0]?.phone2 !== "N/A" && (
															<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
																{selectedItem.CustomerKYC?.[0]?.phone2Status !==
																	"APPROVED" && (
												...
												<div className="flex flex-col gap-4 bg-gray-200 p-4 rounded-lg">
													<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
														<div>
															...
															<p className="font-medium">
																{selectedItem.CustomerKYC?.[0]?.phone3 || "N/A"}
															</p>
														</div>
														...
														<div>
															<p className="text-sm text-default-500">Status</p>
															<p className="font-medium">
																{selectedItem.CustomerKYC?.[0]?.phone3Status ||
																	"N/A"}
															</p>
														</div>
														<div>
													...
													{selectedItem.CustomerKYC?.[0]?.phone3 &&
														selectedItem.CustomerKYC?.[0]?.phone3 !== "N/A" && (
															<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
																{selectedItem.CustomerKYC?.[0]?.phone3Status !==
																	"APPROVED" && (
												...
												<div>
													...
													<p className="font-medium">
														{selectedItem.CustomerKYC?.[0]?.houseNumber ||
															"N/A"}
													</p>
												</div>
												<div>
													...
													<p className="font-medium">
														{selectedItem.CustomerKYC?.[0]?.streetAddress ||
															"N/A"}
													</p>
												</div>
												<div>
													...
													<p className="font-medium">
														{selectedItem.CustomerKYC?.[0]?.nearestBusStop ||
															"N/A"}
													</p>
												</div>
												<div>
													...
													<p className="font-medium">
														{selectedItem.CustomerKYC?.[0]?.localGovernment ||
															"N/A"}
													</p>
												</div>
												<div>
												...
												<div>
													...
													<p className="font-medium">
														{selectedItem.CustomerKYC?.[0]?.updatedAt
															? new Date(
																	selectedItem.CustomerKYC[0].updatedAt
															  ).toLocaleDateString()
															: "N/A"}
										...
										<div className="bg-default-50 p-4 rounded-lg">
											...
											<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
												...
												<div>
													...
													<p className="font-medium">
														{selectedItem.LoanRecord?.[0]?.duration !==
														undefined
															? `${selectedItem.LoanRecord[0].duration} months`
															: "N/A"}
													</p>
												</div>
												<div>
												...
												<div>
													...
													<p className="font-medium">
														{selectedItem.LoanRecord?.[0]?.insurancePackage ||
															"N/A"}
													</p>
												</div>
												<div>
												...
												<div>
													...
													<p className="font-medium">
														{selectedItem.LoanRecord?.[0]?.payFrequency ||
															"N/A"}
													</p>
												</div>
												<div>
												...
												<div>
													...
													<p className="font-medium">
														{selectedItem.LoanRecord?.[0]?.lastPoint || "N/A"}
													</p>
												</div>
												<div>
													<p className="text-sm text-default-500">
													...
													<p className="font-medium">
														{selectedItem.LoanRecord?.[0]?.loanStatus || "N/A"}
													</p>
												</div>
												<div>
													<p className="text-sm text-default-500">Channel</p>
													<p className="font-medium">
														{selectedItem.LoanRecord?.[0]?.channel || "N/A"}
													</p>
												</div>
												<div>
													<p className="text-sm text-default-500">
													...
													<p className="font-medium">
														{selectedItem.LoanRecord?.[0]?.loanRecordId ||
															"N/A"}
													</p>
												</div>
												<div>
												...
												<div>
													...
													<p className="font-medium">
														{selectedItem.LoanRecord?.[0]?.deviceId || "N/A"}
													</p>
												</div>
												<div>
													<p className="text-sm text-default-500">Store ID</p>
													<p className="font-medium">
														{selectedItem.LoanRecord?.[0]?.storeId || "N/A"}
													</p>
												</div>
											</div>
										</div>
			...
			<Modal isOpen={isApproved} onClose={onApprovedClose} size="lg">
				<ModalContent>
					{() => (
						<>
							<ModalHeader>
								Are you sure you want to approve this customer referee?
							</ModalHeader>
							<ModalBody>
								<p className="text-sm text-default-500">
							...
							<ModalFooter className="flex gap-2">
								<Button
									...
									variant="solid"
									onPress={() =>
										handleApproveReferee(selectedItem, refereeType)
									}
			...
			<Modal isOpen={isRejected} onClose={onRejectedClose} size="lg">
				<ModalContent>
					{() => (
						<>
							<ModalHeader>
								Are you sure you want to reject this customer referee?
							</ModalHeader>
							<ModalBody>
								<SelectField
									...
									options={[
										{
											label: "Referee not answering calls",
										...
										},
										{
											...
											value: "Referee advised us to cancel the loan",
										},
										{
