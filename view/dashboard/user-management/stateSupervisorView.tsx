...
import CreateStateSupervisorView from "./createStateSupervisorView";
import PasswordChangeModal from "./components/PasswordChangeModal";
import SuspendUserModal from "./components/SuspendUserModal";
import ViewUserModal from "./components/ViewUserModal";
import type { User, UsersResponse, AccountStatus, stateSupervisor, UserOrClusterSupervisor  } from "./types";

export default function StateSupervisorView() {
  ...
  const users = useMemo(() => {
    ...
    return raw.map((user: stateSupervisor) => ({
      ...user,
      ...
      clusterName: "N/A", // State supervisors don't have clusters
    }));
  }, [raw]);
  ...
  const filtered = useMemo(() => {
    ...
    if (filterValue) {
      ...
      list = list.filter(
        (user: stateSupervisor & { fullName: string }) =>
  ...
  const sorted = React.useMemo(() => {
    return [...paged].sort((a, b) => {
      const aVal = a[sortDescriptor.column as keyof stateSupervisor];
      const bVal = b[sortDescriptor.column as keyof stateSupervisor];
      const cmp = aVal && bVal ? (aVal < bVal ? -1 : aVal > bVal ? 1 : 0) : 0;
      return sortDescriptor.direction === "descending" ? -cmp : cmp;
    });
  }, [paged, sortDescriptor]);

  const exportFn = async (data: (stateSupervisor & { fullName: string; age: string | number; telephoneNumber: string })[]) => {
    ...
    saveAs(new Blob([buf]), "State_Supervisors_Records.xlsx");
  };
  ...
  const renderCell = (row: stateSupervisor & { fullName: string; age: string | number; telephoneNumber: string }, key: string) => {
    if (key === "actions") {
      const dropdownItems = [
        <DropdownItem
          key="view"
  ...
  return (
    <>
      ...
      {isFetchingUsers ? (
        ...
        <GenericTable<stateSupervisor & { fullName: string; age: string | number; telephoneNumber: string }>
          columns={columns}
          data={sorted}
      ...
      >
        <ModalContent>
          ...
          <ModalBody className="p-0 overflow-y-auto">
            <div className="p-6">
              <CreateStateSupervisorView onSuccess={handleCreateSuccess} />
            </div>
          </ModalBody>
        </ModalContent>
      </Modal>
