...
import CreateStateManagerView from "./createStateManagerView";
import PasswordChangeModal from "./components/PasswordChangeModal";
import SuspendUserModal from "./components/SuspendUserModal";
import ViewUserModal from "./components/ViewUserModal";
import type { User, UsersResponse, AccountStatus, stateManager, UserOrClusterSupervisor  } from "./types";

export default function StateManagerView() {
  ...
  const users = useMemo(() => {
    ...
    return raw.map((user: stateManager) => ({
      ...user,
      ...
      clusterName: "N/A", // State managers don't have clusters
    }));
  }, [raw]);
  ...
  const filtered = useMemo(() => {
    ...
    if (filterValue) {
      ...
      list = list.filter(
        (user: stateManager & { fullName: string }) =>
  ...
  const sorted = React.useMemo(() => {
    return [...paged].sort((a, b) => {
      const aVal = a[sortDescriptor.column as keyof stateManager];
      const bVal = b[sortDescriptor.column as keyof stateManager];
      const cmp = aVal && bVal ? (aVal < bVal ? -1 : aVal > bVal ? 1 : 0) : 0;
      return sortDescriptor.direction === "descending" ? -cmp : cmp;
    });
  }, [paged, sortDescriptor]);

  const exportFn = async (data: (stateManager & { fullName: string; age: string | number; telephoneNumber: string })[]) => {
    ...
    saveAs(new Blob([buf]), "State_Managers_Records.xlsx");
  };
  ...
  const renderCell = (row: stateManager & { fullName: string; age: string | number; telephoneNumber: string }, key: string) => {
    if (key === "actions") {
      const dropdownItems = [
        <DropdownItem
          key="view"
  ...
  return (
    <>
      ...
      {isFetchingUsers ? (
        ...
        <GenericTable<stateManager & { fullName: string; age: string | number; telephoneNumber: string }>
          columns={columns}
          data={sorted}
      ...
      >
        <ModalContent>
          ...
          <ModalBody className="p-0 overflow-y-auto">
            <div className="p-6">
              <CreateStateManagerView onSuccess={handleCreateSuccess} />
            </div>
          </ModalBody>
        </ModalContent>
      </Modal>
