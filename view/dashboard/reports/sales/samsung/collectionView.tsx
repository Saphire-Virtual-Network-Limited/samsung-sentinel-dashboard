...
export default function CollectionSamsungView() {
	...
	const filtered = useMemo(() => {
		let list = [...customers].filter(c => c.LoanRecord?.[0]?.loanStatus === 'APPROVED' && c.regBy?.title === 'SAMSUNG PROMOTER');
		if (filterValue) {
	...
	const exportFn = async (data: CustomerRecord[]) => {
		...
		const ws = wb.addWorksheet("Collection Samsung");
		...
		saveAs(new Blob([buf]), "collection_samsung_records.xlsx");
	};
