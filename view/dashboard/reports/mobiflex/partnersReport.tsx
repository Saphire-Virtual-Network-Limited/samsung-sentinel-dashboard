...
export default function MobiflexPartnersReportView() {
	...
	// Fetch data using Mobiflex partner stats hook with date range support
	// Pass period only if no start date is provided (date takes precedence)
	const {
		data: partnerData,
		isLoading,
		error,
	} = useMobiflexPartnerStats(
		startDate ? undefined : selectedPeriod,
		startDate,
		endDate
	);

	// Debug logging to see data structure for different periods
	React.useEffect(() => {
		console.log(
			`${(selectedPeriod || "DATE_RANGE").toUpperCase()} Partner Debug:`,
			{
				partnerData,
				isLoading,
				error,
				partnerCount: partnerData?.partnerStats?.length || 0,
				startDate,
				endDate,
				period: selectedPeriod,
				filterMode: startDate ? "DATE_RANGE" : "PERIOD",
			}
		);
	}, [partnerData, isLoading, error, selectedPeriod, startDate, endDate]);
	...
	const exportFn = async (data: PartnerRecord[]) => {
		...
		saveAs(
			new Blob([buf]),
			`mobiflex_partners_${filterSuffix}_${
