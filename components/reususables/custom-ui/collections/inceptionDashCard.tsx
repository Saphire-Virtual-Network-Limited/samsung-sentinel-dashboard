...
const InceptionDashCard = () => {
	...
	const getChannelData = (channelData: any) => {
		return {
			...
			total_inception_due_loan_value:
				channelData.Total_Inception_Due_Loan_Value || "0",
			average_inception_due_loan_value:
				channelData.Average_Inception_Due_Loan_Value || "0",
			total_inception_mtd_due_loan:
				channelData.Total_Inception_MTD_Due_Loans || 0,
			total_inception_ltd_due_loan:
				channelData.Total_Inception_LTD_Due_Loans || 0,
		};
	};
		...
		return (
			...
			>
				...
				<CardBody className="p-4 space-y-1">
					{renderMetricRow(
						"Total Due Repayments",
						data.total_inception_due_loan
					)}
					{renderMetricRow(
						"Total Due Repayments Value",
						data.total_inception_due_loan_value,
						true
					)}
					{renderMetricRow(
						"Average Due Repayments Value",
						data.average_inception_due_loan_value,
						true
					)}
					{renderMetricRow(
						"This Month MTD Repayments",
						data.total_inception_mtd_due_loan
					)}
					{renderMetricRow(
						"Last Month LTD Repayments",
						data.total_inception_ltd_due_loan
					)}
				</CardBody>
	...
	const renderChannelChart = () => {
		const chartData = [
			{
				name: "MBE",
				value: mbeData.total_inception_due_loan,
				color: "#ef4444",
			},
			{
				name: "Samsung",
				value: samsungData.total_inception_due_loan,
				color: "#8b5cf6",
			},
			{
				name: "Xiaomi",
				value: xiaomiData.total_inception_due_loan,
				color: "#f97316",
			},
			{
				name: "Oppo",
				value: oppoData.total_inception_due_loan,
				color: "#22c55e",
			},
			{
				name: "GLO",
				value: gloData.total_inception_due_loan,
				color: "#3b82f6",
			},
			{
				name: "9Mobile",
				value: nineMobileData.total_inception_due_loan,
				color: "#ec4899",
			},
		];
