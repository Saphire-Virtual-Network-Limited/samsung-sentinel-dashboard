...
const DailyDashCardCollections = () => {
	...
	const metrics = [
		formatMetric(
			...
			formatNumber(overallData.today_due_loan),
			"0",
			"#"
		),
		formatMetric(
			"Total Due Repayments Value",
			formatNumber(overallData.total_due_value),
			"0",
			"#",
			true
		),
		formatMetric(
			...
			formatNumber(overallData.average_due_value),
			"0",
			"#",
			true
		),
		...
		formatMetric(
			...
			formatNumber(overallData.this_month_mtd_due_loan),
			"0",
			"#"
		),
		formatMetric(
			"Last Month LTD Repayments",
			formatNumber(overallData.last_month_ltd_due_loan),
			"0",
			"#"
		),
	];
		...
		return (
			...
			>
				...
				<CardBody className="p-4 space-y-3">
					...
					{renderMetricRow(
						"Total Due Repayments Value",
						data.total_due_value,
						true
					)}
					{renderMetricRow(
						"Average Due Repayments Value",
						data.average_due_value,
						true
					)}
					{renderMetricRow("MTD Repayments", data.this_month_mtd_due_loan)}
					{renderMetricRow(
						"Last Month LTD Repayments",
						data.last_month_ltd_due_loan
					)}
				</CardBody>
...
export default DailyDashCardCollections;
